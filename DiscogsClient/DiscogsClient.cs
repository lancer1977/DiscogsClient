...
namespace DiscogsClient
{
    public class DiscogsClient : IDiscogsDataBaseClient, IDiscogsUserIdentityClient
    {
        ...
        public DiscogsClient(OAuthCompleteInformation oAuthCompleteInformation, string userAgent = null,
            int timeOut = 5000)
        {
            _client = new DiscogsWebClient(oAuthCompleteInformation, userAgent, timeOut);
        }

        public DiscogsClient(TokenAuthenticationInformation tokenAuthenticationInformation, string userAgent = null,
            int timeOut = 5000)
        {
            ...
            CancellationToken token)
        {
            ...
            request.AddJsonBody(new { rating });
            return await _client.Execute<DiscogsReleaseRating>(request, token);
        }
        ...
        public Task<DiscogsSearchResults> SearchAsync(DiscogsSearch search, DiscogsPaginable paginable,
            CancellationToken token)
        {
            RestRequest RequestBuilder()
            {
                return _client.GetSearchRequest().AddAsParameter(search);
            }
        ...
        public IEnumerable<DiscogsReleaseVersion> GetMasterReleaseVersionsAsEnumerable(int masterId, int? max = default)
        {
            return GetMasterReleaseVersions(masterId, max).ToEnumerable();
        }

        public IObservable<DiscogsReleaseVersion> GetMasterReleaseVersions(int masterId, int? max = default)
        {
            var observable = GetMasterReleaseVersionseRaw(masterId, max);
            return max.HasValue ? observable.Take(max.Value) : observable;
        }

        public Task<DiscogsReleaseVersions> GetMasterReleaseVersionsAsync(int masterId, DiscogsPaginable paginable,
            CancellationToken token)
        {
            RestRequest RequestBuilder()
            {
                return _client.GetMasterReleaseVersionRequest(masterId);
            }

            return GetPaginableAsync<DiscogsReleaseVersions>(RequestBuilder, paginable, token);
        }

        public Task<DiscogsReleaseVersions> GetMasterReleaseVersionsAsync(int masterId,
            DiscogsPaginable paginable = null)
        {
            return GetMasterReleaseVersionsAsync(masterId, paginable, CancellationToken.None);
        }

        public IEnumerable<DiscogsArtistRelease> GetArtistReleaseAsEnumerable(int artistId,
            DiscogsSortInformation sort = null, int? max = null)
        {
            return GetArtistRelease(artistId, sort, max).ToEnumerable();
        }

        public IObservable<DiscogsArtistRelease> GetArtistRelease(int artistId, DiscogsSortInformation sort = null,
            int? max = null)
        {
        ...
        public Task<DiscogsArtistReleases> GetArtistReleaseAsync(int artistId, DiscogsSortInformation sort,
            DiscogsPaginable paginable, CancellationToken token)
        {
            RestRequest RequestBuilder()
            {
                return _client.GetArtistReleaseVersionRequest(artistId).AddAsParameter(sort);
            }

            return GetPaginableAsync<DiscogsArtistReleases>(RequestBuilder, paginable, token);
        }

        public Task<DiscogsArtistReleases> GetArtistReleaseAsync(int artistId, DiscogsSortInformation sort = null,
            DiscogsPaginable paginable = null)
        {
        ...
        public Task<DiscogsLabelReleases> GetAllLabelReleasesAsync(int labelId, DiscogsPaginable paginable,
            CancellationToken token)
        {
            RestRequest RequestBuilder()
            {
                return _client.GetAllLabelReleasesRequest(labelId);
            }
        ...
        public Task DownloadImageAsync(DiscogsImage image, Stream copyStream,
            DiscogsImageFormatType type = DiscogsImageFormatType.Normal)
        {
            return DownloadImageAsync(image, copyStream, CancellationToken.None, type);
        }

        public async Task DownloadImageAsync(DiscogsImage image, Stream copyStream, CancellationToken cancellationToken,
            DiscogsImageFormatType type = DiscogsImageFormatType.Normal)
        {
        ...
        public Task<string> SaveImageAsync(DiscogsImage image, string path, string fileName,
            DiscogsImageFormatType type = DiscogsImageFormatType.Normal)
        {
            return SaveImageAsync(image, path, fileName, CancellationToken.None, type);
        }

        public async Task<string> SaveImageAsync(DiscogsImage image, string path, string fileName,
            CancellationToken cancellationToken, DiscogsImageFormatType type = DiscogsImageFormatType.Normal)
        {
            var url = type == DiscogsImageFormatType.Normal ? image.uri : image.uri150;
        ...
        private IObservable<DiscogsReleaseVersion> GetMasterReleaseVersionseRaw(int masterId, int? max = default)
        {
            RestRequest RequestBuilder()
            {
                return _client.GetMasterReleaseVersionRequest(masterId);
        ...
        private IObservable<DiscogsArtistRelease> GetArtistReleaseRaw(int artistId, DiscogsSortInformation sort = null,
            int? max = null)
        {
            RestRequest RequestBuilder()
            {
        ...
        private IObservable<T> GenerateFromPaginable<T, TRes>(Func<RestRequest> requestBuilder, int? max = null)
            where T : DiscogsEntity
        ...
        private async Task<T> GetPaginableAsync<T>(Func<RestRequest> requestBuilder, DiscogsPaginable paginable,
            CancellationToken token)
        {
