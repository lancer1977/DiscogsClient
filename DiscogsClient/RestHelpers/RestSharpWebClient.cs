...
namespace DiscogsClient.RestHelpers
{
    public abstract class RestSharpWebClient
    {
        ...
        protected IRestClient Client => _Client ?? (_Client = GetClient(UrlBase, _TimeOut));

        public string UserAgent
        {
            get => _UserAgent ?? UserAgentFallBack;
        ...
        protected IRestClient GetClient(string urlBase, int timeOut = 10000)
        {
            var client = new RestClient(new RestClientOptions
                { UserAgent = UserAgent, Timeout = TimeSpan.FromSeconds(10), BaseUrl = new Uri(urlBase) });
        ...
        private async Task<RestResponse<T>> GetResponse<T>(RestRequest request, CancellationToken cancellationToken,
            IRestClient client = null)
        {
            client = client ?? Client;
            var response =
                await TimeLimiter.Enqueue(async () => await ExecuteBasic<T>(client, request, cancellationToken),
                    cancellationToken);
        ...
        private static Task<RestResponse<T>> ExecuteBasic<T>(IRestClient client, RestRequest request,
        ...
        public async Task<string> SaveFile(string url, string path, string fileName,
            CancellationToken cancellationToken, int timeOut = 15000)
        {
        ...
        private async Task<RestResponse> GetResponse(RestRequest request, CancellationToken cancellationToken,
            IRestClient client = null)
        {
            client = client ?? Client;
            var response = await TimeLimiter.Enqueue(async () => await ExecuteBasic(client, request, cancellationToken),
                cancellationToken);
